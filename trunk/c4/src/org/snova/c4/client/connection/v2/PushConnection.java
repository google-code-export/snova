...
class PushConnection extends HTTPPersistentConnection
{
	...
	private void finishQueuedContens()
	{
		...
		for (Buffer c : queuedContents)
		{
			...
			ChannelBuffer tmp = ChannelBuffers.wrappedBuffer(
			        c.getRawBuffer(), c.getReadIndex(), c.readableBytes());
			if(null == wrap)
			{
				wrap = tmp;
			}
		...
		final HttpChunk chunk = new DefaultHttpChunk(
		        ChannelBuffers.wrappedBuffer(sizeBuf, wrap));
	...
	private synchronized void sendContent(Buffer content)
	{
		if (!waitingResponse.get())
		{
			...
			boolean isPullEnabled = C4ClientConfiguration.getInstance()
			        .isClientPullEnable();
			if (isPullEnabled)
			{
				...
				SharedObjectHelper.getGlobalTimer().schedule(
				        ...
				        C4ClientConfiguration.getInstance()
				                .getPullTransactionTime(), TimeUnit.SECONDS);
			}
			else
			{
				
				ChannelBuffer sent = ChannelBuffers.wrappedBuffer(
				...
				request.setHeader("Content-Length",
				        String.valueOf(tmp.readableBytes()));
	...
	protected void onFullResponseReceived()
	{
		...
		{
			// Buffer content = queuedContents.removeFirst();
			send(null);
		}
	}
