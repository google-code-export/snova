...
/**
 * Decodes {@link ByteBuf}s into {@link SocksInitRequest}. Before returning
 * SocksRequest decoder removes itself from pipeline.
 */
public class SocksInitRequestDecoder extends
        ReplayingDecoder<SocksInitRequestDecoder.State>
{
	...
	@Override
	public Object decode(ChannelHandlerContext ctx, Channel ch,
	        ChannelBuffer byteBuf, State state) throws Exception
	{
		switch (state)
		{
			...
			case READ_AUTH_SCHEMES:
			{
				...
				{
					authSchemes.add(SocksMessage.AuthScheme.fromByte(byteBuf
					        .readByte()));
				}
