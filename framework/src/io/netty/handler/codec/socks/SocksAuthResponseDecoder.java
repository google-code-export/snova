...
/**
 * Decodes {@link ByteBuf}s into {@link SocksAuthResponse}. Before returning
 * SocksResponse decoder removes itself from pipeline.
 */
public class SocksAuthResponseDecoder extends
        ReplayingDecoder<SocksAuthResponseDecoder.State>
{
	...
	@Override
	public SocksResponse decode(ChannelHandlerContext channelHandlerContext,
	        Channel channel, ChannelBuffer byteBuf, State state)
	        throws Exception
	{
		switch (state)
		{
			...
			{
				authStatus = SocksMessage.AuthStatus.fromByte(byteBuf
				        .readByte());
